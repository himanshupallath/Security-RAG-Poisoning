...
namespace CosmosRecipeGuide
{
    internal class Program
    {
        ...
        static List<ChatRequestMessage> messageHistory = new List<ChatRequestMessage>();
        ...
        static async Task Main(string[] args)
        {
            ...
            const string controlSearch = "4.\tAsk AI Assistant (Control)";
            const string dontSaveSearch = "5.\tAsk AI Assistant (Don't Save)";
            const string saveSanitizedSearch = "6.\tAsk AI Assistant (Save Sanitized)";
            const string exit = "7.\tExit this Application";
            ...
            while (true)
            {
                var selectedOption = AnsiConsole.Prompt(
                      new SelectionPrompt<string>()
                          ...
                          .AddChoices(new[] {
                            createContainer, cosmosUpload, vectorize, controlSearch, dontSaveSearch, saveSanitizedSearch, exit
                          }));
                ...
                switch (selectedOption)
                {
                    ...
                    case vectorize:
                        ...
                        break;
                    case controlSearch:
                        await PerformSearch(config, SearchMode.Control);
                        break;
                    case dontSaveSearch:
                        await PerformSearch(config, SearchMode.DontSave);
                        break;
                    case saveSanitizedSearch:
                        await PerformSearch(config, SearchMode.SaveSanitized);
                        break;
                    case exit:
                        return;
                }
        ...
        private static void createCosmosContainer(IConfiguration config)
        {
            ...
            AnsiConsole.Status()
               ...
               {
                   isSuccess = cosmosService.CreateCosmosContainerAsync(databaseName, containerName).GetAwaiter().GetResult();
               });
        ...
        private static async Task PerformSearch(IConfiguration config, SearchMode searchMode)
        {
            ...
            while (true)
            {
                string userQuery = Console.Prompt(
                    new TextPrompt<string>("Type your question (or type 'quit' to exit chat), hit enter when ready.")
                        .PromptStyle("teal")
                );

                if (userQuery.ToLower() == "quit")
                {
                    break;
                }

                string sanitizedQuery = userQuery;
                bool rejected = false;

                if (searchMode == SearchMode.DontSave || searchMode == SearchMode.SaveSanitized)
                {
                    await AnsiConsole.Status()
                       .StartAsync("Sanitizing prompt...", async ctx =>
                       {
                           ...
                           sanitizedQuery = await openAIEmbeddingService.SanitizePromptAsync(userQuery);
                       });

                    if (sanitizedQuery.Trim().ToUpper() == "REJECTED")
                    {
                        rejected = true;
                    }
                    else if (sanitizedQuery != userQuery)
                    {
                        AnsiConsole.MarkupLine($"[yellow]Your prompt was sanitized. The AI will answer the following prompt:[/] {sanitizedQuery}");
                    }
                }

                if (rejected)
                {
                    chatCompletion = "[red]Your prompt was rejected by the guardrail model.[/]";
                    if (searchMode == SearchMode.DontSave)
                    {
                        messageHistory.Add(new ChatRequestUserMessage(userQuery));
                        messageHistory.Add(new ChatRequestAssistantMessage("REJECTED"));
                    }
                }
                else
                {
                    await AnsiConsole.Status()
                       .StartAsync("Processing...", async ctx =>
                       {
                           ...
                           var embeddingVector = await openAIEmbeddingService.GetEmbeddingsAsync(sanitizedQuery);

                           ctx.Status("Performing Vector Search.. Retrieving recipe(s) from Cosmos DB (RAG pattern)..");
                           var retrivedDocs = await cosmosService.SingleVectorSearch(embeddingVector, 0.60);
                           ...
                           {
                               recipe.vectors = null;
                               retrivedReceipeNames += ", " + recipe.name;
                           }

                           ctx.Status($"Processing '{retrivedReceipeNames}' to generate Completion using OpenAI Service..");

                           var requestMessage = searchMode == SearchMode.SaveSanitized ? new ChatRequestUserMessage(sanitizedQuery) : new ChatRequestUserMessage(userQuery);

                           (string completion, int promptTokens, int completionTokens) = await openAIEmbeddingService.GetChatCompletionAsync(sanitizedQuery, JsonConvert.SerializeObject(retrivedDocs), messageHistory);
                           chatCompletion = completion;

                           messageHistory.Add(requestMessage);
                           messageHistory.Add(new ChatRequestAssistantMessage(chatCompletion));
                       });
                }
